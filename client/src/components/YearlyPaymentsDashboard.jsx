...
export default function YearlyPaymentsDashboard({ projects = [], onOpenProject }) {
  ...
  const [displayCurrency, setDisplayCurrency] = useState(() => {
    return localStorage.getItem('yearly_display_currency') || 'TRY';
  });
  const [hideAmounts, setHideAmounts] = useState(() => {
    return localStorage.getItem('yearly_hide_amounts') === 'true';
  });
  ...
  useEffect(() => {
    localStorage.setItem('yearly_display_currency', displayCurrency);
  }, [displayCurrency]);

  useEffect(() => {
    localStorage.setItem('yearly_hide_amounts', String(hideAmounts));
  }, [hideAmounts]);
  ...
  const totalReceivable = payments.reduce((sum, p) => sum + getPaymentTotal(p, displayCurrency), 0);

  const categoryBreakdown = payments.reduce((acc, p) => {
  ...
  const breakdownList = Object.entries(categoryBreakdown)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
        ...
        >
          ...
          <div className="stat-value" style={{ fontSize: '28px', color: 'var(--success)' }}>{fmt(totalReceivable)}</div>
          <div className="stat-desc" style={{ color: 'var(--text-muted)' }}>
            Tüm kalemlerin seçili kurdaki toplamı. Kırılım grafiklerini görmek için tıklayın.
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state glass-card" style={{ padding: '60px 40px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '8px', maxWidth: '300px', marginInline: 'auto' }}>
            Yukarıdaki butondan veya Projeler modalındaki aktarma butonundan yıllık ödeme ekleyebilirsiniz.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-main)' }}>{payment.title}</h3>
                    {payment.client && (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                    <strong style={{ fontSize: '18px', fontFamily: 'Fira Code, monospace', color: 'var(--primary)' }}>
                      {fmt(totalInDisplay)}
                    </strong>
                    <span
                      style={{
      ...
      {isBreakdownModalOpen && (
        <div className="modal-backdrop open" onClick={() => setIsBreakdownModalOpen(false)}>
          <div className="modal glass-card" style={{ maxWidth: '500px' }} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><BarChart2 style={{ color: 'var(--success)' }} /> Kalem Bazlı Kırılım (Yıllık)</h2>
            ...
            <div className="modal-body" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {breakdownList.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', textAlign: 'center', fontSize: '13px' }}>Henüz hiçbir kalem kaydedilmemiş.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  {breakdownList.map((item, idx) => {
                    const percentage = totalReceivable > 0 ? (item.amount / totalReceivable) * 100 : 0;
                    return (
                  ...
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '14px', marginTop: '10px', display: 'flex', justifyContent: 'space-between', fontSize: '16px', fontWeight: 'bold' }}>
                    ...
                    <span style={{ color: 'var(--success)' }}>{fmt(totalReceivable)}</span>
                  </div>
                </div>
              )}
            </div>
